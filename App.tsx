...
const normalizeDialogueHistory = (raw: any): ClaudeChatHistory | null => {
  if (!raw || !Array.isArray(raw.messages)) return null;

  const normalizedMessages = raw.messages.map((msg: any) => {
    const normalizedRole = msg?.role === 'user' ? 'user' : 'assistant';

    const normalizedContent = Array.isArray(msg?.content)
      ? msg.content.map((part: any) => (typeof part === 'object' && part !== null ? part : { type: 'text', text: String(part ?? '') }))
  ...
  return {
    ...
    system: Array.isArray(raw.system) ? raw.system : [],
...
const App: React.FC = () => {
  ...
  const handleParse = useCallback(() => {
    ...
    try {
      ...
      const normalized = normalizeDialogueHistory(json);
      if (normalized) {
