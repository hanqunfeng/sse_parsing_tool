...
const EventItem: React.FC<EventItemProps> = ({ event }) => {
  ...
  const eventLabel = event.parsedData?.object === 'chat.completion.chunk'
    ? 'chunk'
    : event.event === 'done'
      ? 'done'
      : event.event;
  ...
  return (
    <div className="border-b border-slate-200 last:border-0">
      ...
      >
        <span className={`px-2 py-0.5 rounded text-[10px] font-bold uppercase tracking-wider border ${getEventColor(eventLabel)}`}>
          {eventLabel}
        </span>
